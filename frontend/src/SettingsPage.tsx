...
const defaultSettings: SettingsValue = {
  reminderDays: [30, 14, 7, 3, 1, 0], reminderEnabled: true, defaultCurrency: 'CNY',
  timezone: 'Asia/Shanghai', smtpHost: '', smtpPort: 587, smtpUser: '', smtpFrom: '', smtpTo: ''
};
...
export function SettingsPage() {
  ...
  const saveSettings = async (event: FormEvent) => {
    ...
    const r = await api.put<{ settings: SettingsValue }>('/api/settings', settings);
    setSettings(r.settings); setMessage('设置已保存');
  };

  const testEmail = async () => {
    setMessage('');
    try { await api.post('/api/settings/test-email'); setMessage('测试邮件已发送'); }
    catch (err) { setMessage(err instanceof ApiError ? err.message : '测试邮件失败'); }
  };

  const runReminder = async () => {
    const r = await api.post<{ sent: boolean; items: unknown[] }>('/api/reminders/run-now');
    setMessage(r.sent ? `已发送提醒：${r.items.length} 项` : '没有需要发送的新提醒');
  };

  const changePassword = async (event: FormEvent) => {
    event.preventDefault();
    await api.put('/api/auth/password', passwordForm);
    setPasswordForm({ currentPassword: '', newPassword: '' }); setMessage('密码已修改');
  };
        ...
        <form onSubmit={saveSettings} className="space-y-4">
          <div className="grid gap-3 md:grid-cols-2">
            <Field label="提醒天数，逗号分隔">
              <input className={inputClass} value={settings.reminderDays.join(',')} onChange={(e) => setSettings({ ...settings, reminderDays: e.target.value.split(',').map((v) => Number(v.trim())).filter((v) => Number.isFinite(v)) })} />
            </Field>
            <Field label="提醒开关">
              <select className={inputClass} value={String(settings.reminderEnabled)} onChange={(e) => setSettings({ ...settings, reminderEnabled: e.target.value === 'true' })}>
                <option value="true">Enabled</option><option value="false">Disabled</option>
              </select>
            </Field>
            <Field label="默认币种">
              <select className={inputClass} value={settings.defaultCurrency} onChange={(e) => setSettings({ ...settings, defaultCurrency: e.target.value as SettingsValue['defaultCurrency'] })}>
                <option>CNY</option><option>USD</option><option>GBP</option><option>EUR</option>
              </select>
            </Field>
          ...
          <div className="flex flex-wrap gap-2">
            <Button type="submit"><Save size={15} />保存设置</Button>
            <Button type="button" variant="secondary" onClick={testEmail}><Mail size={15} />测试邮件</Button>
            <Button type="button" variant="secondary" onClick={runReminder}><Play size={15} />手动扫描</Button>
            <a className="inline-flex h-9 items-center gap-2 rounded-md border border-zinc-200 px-3 text-sm text-zinc-600 hover:bg-zinc-100 dark:border-zinc-800 dark:text-zinc-300 dark:hover:bg-zinc-800" href="/api/export/json">
