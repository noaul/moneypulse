...
import { Button, DataTable, Drawer, Field, Skeleton, inputClass, type DataTableColumn } from './ui';

type ExpenseForm = { assetType: AssetType; assetId: string; amount: string; currency: Currency; paidAt: string; periodStart: string; periodEnd: string; category: 'renewal' | 'monthly' | 'setup' | 'other'; notes: string };
const initialForm: ExpenseForm = { assetType: 'subscription', assetId: '', amount: '', currency: 'CNY', paidAt: new Date().toISOString().slice(0, 10), periodStart: '', periodEnd: '', category: 'monthly', notes: '' };
...
export function Expenses() {
  ...
  const remove = async (item: ExpenseItem) => { if (!window.confirm('删除这条费用流水？')) return; await api.delete(`/api/expenses/${item.id}`); await load(); };

  const columns: DataTableColumn<ExpenseItem>[] = [
    { key: 'asset', header: '资产', render: (item) => <div><span className="font-semibold">{item.assetType}</span><span className="ml-2 font-mono text-xs text-slate-400">#{item.assetId}</span></div> },
    ...
    { key: 'period', header: '覆盖周期', align: 'right', render: (item) => <span className="text-slate-500">{compactDate(item.periodStart)} → {compactDate(item.periodEnd)}</span> },
    { key: 'cat', header: '分类', render: (item) => <span className="rounded-lg bg-slate-100 px-2 py-0.5 text-xs font-medium text-slate-600 dark:bg-slate-800 dark:text-slate-400">{item.category}</span> },
    { key: 'actions', header: '', align: 'right', render: (item) => (
      <div className="flex justify-end gap-1">
        ...
        <button className="rounded-lg p-1.5 text-slate-400 hover:bg-danger-50 hover:text-danger-500" onClick={() => remove(item)}><Trash2 size={14} /></button>
      </div>
    )}
  ];
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold tracking-tight">费用流水</h2>
          <p className="mt-1 text-sm text-slate-500">记录真实付款，Dashboard 实际支出从这里统计。</p>
        </div>
        <Button onClick={openCreate}><Plus size={16} />新增流水</Button>
      </div>

      {loading ? <Skeleton className="h-64 rounded-2xl" /> : <DataTable columns={columns} data={items} emptyText="暂无费用流水" />}
